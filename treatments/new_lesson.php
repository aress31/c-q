<?php  
 ...
 /*On n'effectue les traitement qu'à la condition que les informations aient été effectivement postées*/
    if (isset($_POST) && (!empty($_POST['lesson_title'])) && (!empty($_POST['lesson_theme'])) && (!empty($_POST['part_title']))) {
   
	  extract($_POST);  
	  
	  $lesson_title = mysql_real_escape_string($lesson_titile);
	  $part_title = mysql_real_escape_string($part_titile);
	  $lesson_theme = mysql_real_escape_string($lesson_theme);
		...
		if ($_FILES['my_file']['size'] > $max_file_size) {
			...
			$sql = ("SELECT titleL FROM Lessons WHERE titleL = '".$lesson_title."' ");
			$req = mysql_query($sql) or die(mysql_error()) ;
			
			if(mysql_num_rows($req) == 0)
				$sql = ("INSERT INTO Lessons (titleL, nomTh)
					VALUES ('".$lesson_title."','" .$lesson_theme."') ");
